...
- A `manifest.json` file exists in `public/` with `name`, `short_name`, `start_url`, `display`, `background_color`, `theme_color`, and `icons` fields
...
- Icons are referenced in `manifest.json` with correct `src`, `sizes`, and `type`
...
- Icons are derived from the existing `favicon.svg` design (blue background, white "K")
...
- Cache versioning allows the service worker to update cached resources when the blog is redeployed
