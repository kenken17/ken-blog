...
| Story 2: PWA Icons | Generate and reference PWA icons at required sizes so the installed app looks native | 192x192, 512x512, and 180x180 (Apple) icons exist; referenced in manifest and BaseLayout |
| Story 3: Service Worker for Offline Access | Implement a service worker with cache-first for assets and network-first for pages so users can read offline | Service worker caches assets and pages; serves offline; registered in BaseLayout; cache versioning works |
